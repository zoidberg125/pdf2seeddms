...
# ------------------------------------------------------------------------------
    ...
    def __tr_params(self, params):
        ...
        for keyname, keyvalue in params.items():
            if keyvalue is None:
                retv[keyname] = ''
                continue
            ...
            print("unhandled type %s for %s".format(type(keyvalue), keyname))
...
# ------------------------------------------------------------------------------
    ...
    def rest_post(self, url, data, argdict=None, params=None):
        ...
        if params is None:
            ...
            params = self.__tr_params(params)
            files = {'file': data}
            req_obj = requests.post(url, files=files, params=params, cookies=self.cookies)
        return SeedDMSData(req_obj)
